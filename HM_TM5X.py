...
def packetTemplate(class_addr, subclass_addr, rw_flag, data, size, chk):
    vals = [BEGIN, size, DEVICE_ADDR, class_addr, subclass_addr, rw_flag, data, chk, END]
    output = f'0x'
    for val in vals:
        output += f'{val:02X}'
    return output
...
def parseAutoShutterControl(feedback):
    ...
    values = ['Automatic control off', 'Automatic switching, timing control',
              'Automatic switch, temperature difference control', 'Full-automatic control (default)']
...
def parsePalette(feedback):
    ...
    values = ["White Hot", "Black Hot", "Fusion 1", "Rainbow", "Fusion 2", "Iron Red 1", "Iron Red 2",
              "Dark Brown", "Color 1", "Color 2", "Ice Fire", "Rain", "Green Hot", "Red Hot", "Deep Blue"]
    return values[int(data)]
...
def parseImageMirroring(feedback):
    ...
    if int(data) < 0 or int(data) > 0x03:
        print(f"data must be between 0x00 and 0x03, got {data}")
        return f"-1 data must be between 0x00 and 0x03, got {data}"
    values = ["no mirroring", "central mirroring", "left/right mirroring", "up/down mirroring"]
    return values[int(data)]
